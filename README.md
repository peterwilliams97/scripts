...
| Script | What it does |
| --- | --- |
| `current_branches.sh` | Lists local branches oldest-commit-first with a `MERGED` column that catches squash merges, not just fast-forward ancestry. Takes the clone to inspect as a positional (default `.`), told apart from the base-branch positional by whether it names a directory. `-clean` deletes the merged ones; `-update` pulls the base branch into every branch; `--poll <sec>` redraws on a timer to watch another process commit. |
